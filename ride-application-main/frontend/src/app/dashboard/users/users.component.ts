...
export class UsersComponent {
  ...
  countrycode: any[] = [];
  file: any;
  selectedCC: any = '+91'; // Initialize with default value
  id: any;
  count: any;
  search: string = '';
  usersArray: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.AddForm = this.formBuilder.group({
      ...
      countrycode: [this.selectedCC, [Validators.required]],
  ...
  getcountryCode(){
    this._users.getcode().subscribe({
      next: (response) => {
        ...
        if (response && response.countrydata) {
          this.countrycode = []; // Clear existing array
          ...
          this.countrycode.sort();
          console.log('All country codes:', this.countrycode);
        } else {
  ...
  onSelected(value: any) {
    ...
    // Update the form control value for add form
    if (this.AddbuttonForm) {
      this.AddForm.patchValue({
        countrycode: value
      });
    }
    // Update the form control value for update form
    if (this.updateForm) {
  ...
  AddUser() {
    ...
    formData.append("countrycode", this.AddForm.value.countrycode || this.selectedCC);
  ...
  toggleFormVisibility() {
    ...
    this.AddForm.reset();
    this.selectedCC = '+91'; // Reset selected country code
    this.AddForm.patchValue({
      countrycode: this.selectedCC
    });
  }
